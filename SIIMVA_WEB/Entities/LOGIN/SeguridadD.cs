...
//using NotificadiorCiDiApi.Entities.CIDI;
...
namespace MOTOR_WORKFLOW.Entities.LOGIN
{
    public class SeguridadDal : DALBase
    {
        ...
        private static List<Usuario_V2> mapeo(SqlDataReader dr)
        {
            List<Usuario_V2> lst = new List<Usuario_V2>();
            Usuario_V2 obj;
            if (dr.HasRows)
            {
                while (dr.Read())
        ...
        public static List<Usuario_V2> read()
        {
        ...
        public async static Task<Usuario_V2> readUser(string user)
        {
            try
            {
                string strSQL = @"SELECT cod_usuario, nombre, passwd, nombre_completo,
